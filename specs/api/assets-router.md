...
|-------|-----------|-------|
| `stones` | SELECT, INSERT, UPDATE | Primary asset table. No DELETE — assets are archived. |
...
```typescript
z.object({
  valuePath: z.enum(['all', 'fractional', 'tokenization', 'debt']).default('all'),
  assetType: z.enum(['all', 'gemstone', 'real_estate', 'precious_metal', 'mineral_rights', 'other']).default('all'),
  status: z.enum(['all', 'active', 'blocked', 'on_hold', 'complete', 'terminated']).default('all'),
  teamMemberId: z.string().uuid().optional(),
  search: z.string().optional(),
  sortBy: z.enum(['name', 'reference_code', 'created_at', 'current_phase', 'estimated_value']).default('created_at'),
...
**Query Pattern:**
- SELECT from `stones` WHERE id = assetId
...
**Query Pattern:**
- Aggregation query on `stones` WHERE status IN ('active', 'in_progress')
...
z.object({
  ...
  valuePath: z.enum(['fractional', 'tokenization', 'debt']).optional(),
  holder: z.union([
...
3. INSERT into `stones`:
   - `name`, `asset_type`, `reference_code`, `value_path`
   - `estimated_value`, `source_channel`, `description`
...
**Mutation Pattern:**
- UPDATE `stones` SET fields WHERE id = assetId
...
z.object({
  assetId: z.string().uuid(),
  newPhase: z.number().int().min(1).max(4),
...
- UPDATE `stones` SET `current_phase` = newPhase WHERE id = assetId
...
**Mutation Pattern:**
- UPDATE `stones` SET `status` = 'archived', `metadata.archive_reason` = reason
...
**Query:** `SELECT COUNT(*) FROM stones WHERE reference_code = $1`
...
Implementation: Supabase Realtime is handled at the database level via NOTIFY/LISTEN on the `stones` table. Frontend subscribes to the `stones` table changes.
...
- **"asset" not "stone":** API response types use "asset" naming even though DB table is "stones"
