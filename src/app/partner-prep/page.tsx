...
export default function PartnerPrep() {
  ...
  const brickkenQuestions: QA[] = [
    { q: "Which tier should we start with for our first $10M offering?", why: "We need to understand if Advanced ($5K/yr, EUR 5M cap) is enough or if we need Professional/Enterprise. Our first offering is ~$10M.", ourAnswer: "We're looking at Advanced to start, but our pipeline is $1B+. We want to understand the upgrade path as we scale." },
    { q: "Does your platform handle cross-sheet compliance for US Reg D 506(c) offerings?", why: "We need to know if Brickken can enforce accredited investor requirements and transfer restrictions at the smart contract level.", ourAnswer: "We're offering exclusively under Reg D 506(c) to accredited investors. We need on-chain KYC whitelisting and transfer restrictions." },
    { q: "How does your KYC integration work? Is it included in the subscription?", why: "We need to know if the 150 KYC checks in Advanced tier are sufficient, and what providers they use.", ourAnswer: "We expect 50-200 investors across our first few offerings. We need KYC + accredited investor verification." },
    { q: "Do you deploy ERC-3643 tokens? What token standard do you use?", why: "ERC-3643 is the institutional standard for security tokens ($32B+ tokenized). We need permissioned transfers.", ourAnswer: "We specifically need ERC-3643 on Polygon for institutional credibility and built-in compliance." },
    ...
    { q: "Which blockchains do you support? Can we deploy on Polygon?", why: "We chose Polygon for low gas costs and institutional adoption.", ourAnswer: "Polygon is our target chain. Low gas, Brickken-supported, Chainlink PoR available." },
    { q: "Are your smart contracts already audited? By whom?", why: "If pre-audited, we only need a config review ($3-5K), not a full audit ($50-100K).", ourAnswer: "We want to leverage your pre-audited contracts to avoid the cost of a full custom audit." },
    { q: "How do you handle the cap table / shareholder registry?", why: "We need to know if Brickken can serve as the transfer agent function via on-chain registry.", ourAnswer: "Per the SEC's January 2026 statement, blockchain can serve as the master securityholder file. Can your platform fulfill this role?" },
    ...
    { q: "Can we white-label the investor-facing interface?", why: "We need PleoChrome branding, not Brickken branding, on what investors see.", ourAnswer: "Our brand is 'institutional luxury.' The investor portal needs to match our design system — navy + gem accents." },
  ...
  const whatWeTell: { topic: string; script: string }[] = [
    ...
    { topic: "Our Differentiator", script: "Three things make PleoChrome different: (1) The 3-Appraisal Rule — every stone gets three independent USPAP-compliant appraisals and we use the two lowest to set the offering price. Structurally conservative. (2) Oracle-gated minting via Chainlink Proof of Reserve — tokens physically cannot be created unless the oracle confirms vault custody. (3) ERC-3643 compliance tokens — every transfer is verified at the smart contract level. No exceptions." },
    ...
    { topic: "What We Need from Brickken", script: "We need a tokenization partner, not a vendor. We need: ERC-3643 deployment on Polygon, built-in KYC/accreditation verification, Chainlink PoR integration for mint-gating, a clean investor onboarding experience, and the ability to scale from one offering to hundreds. We're evaluating Brickken, Securitize, and Tokeny — and we want to move fast with the right partner." },
  ...
  const sections = [
    { id: "brickken", title: "Brickken Meeting Prep", subtitle: "16 questions + talking points for tomorrow", color: "#1A8B7A", count: brickkenQuestions.length },
          ...
          <>
            <div className={`${cd} border rounded-2xl p-4 sm:p-5 mb-5`}>
              <p className={`text-xs sm:text-sm ${s2} leading-relaxed`}>
                Use this before ANY partner meeting. It has: specific questions for Brickken (tomorrow&apos;s call),
                scripted talking points about PleoChrome, and a universal question set that works for any partner
                (appraisers, vaults, broker-dealers, attorneys).
              </p>
            </div>
        ...
        {activeSection === "brickken" && (
          <>
            ...
            <div className={`${cd} border rounded-2xl p-4 sm:p-5 mb-4`}>
              <h2 className={`text-sm font-semibold ${s3} mb-1`}>Brickken Meeting — Question Playbook</h2>
