...
export default function InterestPageContent() {
        ...
        <div className="max-w-2xl mx-auto">
          {submitted ? (
            <div className="text-center py-20">
              ...
              <Link href="/knowledge" className="text-sm text-[#1A8B7A] hover:text-white transition-colors flex items-center justify-center gap-1">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={form.fullName} onChange={e => setForm(p => ({ ...p, fullName: e.target.value }))} placeholder="Your full name" className={inputClass} />
                ...
                <div>
                  ...
                  <input type="text" value={form.entity} onChange={e => setForm(p => ({ ...p, entity: e.target.value }))} placeholder="Company, family office, or fund name" className={inputClass} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {assetOptions.map(asset => (
                      <button key={asset} type="button" onClick={() => toggleAsset(asset)}
                        className={`px-3 py-1.5 rounded-full text-xs border transition-all ${form.assetInterests.includes(asset) ? 'border-[#1A8B7A] bg-[#1A8B7A]/10 text-[#1A8B7A]' : 'border-white/10 text-white/40 hover:border-white/20'}`}>
                        {asset}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <input type="text" value={form.referralSource} onChange={e => setForm(p => ({ ...p, referralSource: e.target.value }))} placeholder="LinkedIn, referral, conference, etc." className={inputClass} />
                </div>
