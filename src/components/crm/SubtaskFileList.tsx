...
interface SubtaskFileListProps {
  subtaskId: string
  taskId: string
  assetId?: string
}
...
export function SubtaskFileList({ subtaskId, taskId, assetId }: SubtaskFileListProps) {
  const [uploading, setUploading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const { toast } = useToast()
  const utils = trpc.useUtils()

  const { data: docs = [] } = trpc.documents.listBySubtask.useQuery({ subtaskId })
  const createDoc = trpc.documents.create.useMutation({
    onSuccess: () => {
      utils.documents.listBySubtask.invalidate({ subtaskId })
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const path = `subtasks/${subtaskId}/${Date.now()}-${file.name}`
