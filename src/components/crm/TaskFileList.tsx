...
export function TaskFileList({ taskId, assetId }: TaskFileListProps) {
  const [uploading, setUploading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const { toast } = useToast()
  const utils = trpc.useUtils()

  const { data: docs = [] } = trpc.documents.listByTask.useQuery({ taskId })
  const createDoc = trpc.documents.create.useMutation({
    onSuccess: () => { utils.documents.listByTask.invalidate({ taskId }); toast('File uploaded', 'success') },
    onError: (err) => toast(err.message, 'error'),
  })
  const deleteDoc = trpc.documents.delete.useMutation({
    onSuccess: () => { utils.documents.listByTask.invalidate({ taskId }); toast('File deleted', 'success') },
    onError: (err) => toast(err.message, 'error'),
  })
