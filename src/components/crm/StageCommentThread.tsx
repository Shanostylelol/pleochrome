...
export function StageCommentThread({ stageId, assetId, currentUserId }: StageCommentThreadProps) {
  const utils = trpc.useUtils()
  const { toast } = useToast()
  const { data: rawComments = [] } = trpc.comments.listByEntity.useQuery({ stageId })

  const postComment = trpc.comments.create.useMutation({
    onSuccess: () => { utils.comments.listByEntity.invalidate({ stageId }); toast('Comment posted', 'success') },
    onError: (err) => toast(err.message, 'error'),
  })
  const deleteComment = trpc.comments.delete.useMutation({
    onSuccess: () => { utils.comments.listByEntity.invalidate({ stageId }); toast('Comment deleted', 'success') },
    onError: (err) => toast(err.message, 'error'),
  })
  ...
  return (
    <CommentThread
      ...
      onPost={(body) => postComment.mutate({ stageId, assetId, body })}
      onReply={(parentId, body) => postComment.mutate({ stageId, assetId, body, parentCommentId: parentId })}
