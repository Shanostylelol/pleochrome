...
function DrawerContent({ onClose, pathname }: { onClose: () => void; pathname: string }) {
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed top-0 right-0 bottom-0 w-72 bg-[#0A0F1A] border-l border-white/10 overflow-y-auto"
  ...
  return (
    <div className="sm:hidden">
      ...
      {open && mounted && createPortal(
        <DrawerContent onClose={() => setOpen(false)} pathname={pathname} />,
        document.body
      )}
    </div>
  )
